...
async def handle_blogger(callback_query: CallbackQuery, state: FSMContext):
    logging.info("Обработчик 'handle_blogger' вызван.")  # Логирование
    await callback_query.message.edit_text(
        "Вы выбрали работать блоггером! Для регистрации\nВведите ваше ФИО:"
    )
    await state.set_state(RegisterBlogger.waiting_for_name)

@router.message(StateFilter(RegisterBlogger.waiting_for_name))
async def process_name_input(message: Message, state: FSMContext):
    ...
    await state.set_state(RegisterBlogger.waiting_for_email)

@router.message(StateFilter(RegisterBlogger.waiting_for_email))
async def process_email_input(message: Message, state: FSMContext):
    ...
    await state.set_state(RegisterBlogger.waiting_for_inst)

@router.message(StateFilter(RegisterBlogger.waiting_for_inst))
async def process_inst_input(message: Message, state: FSMContext):
    inst = message.text
    await state.update_data(inst=inst)
    await state.update_data(role="blogger")
    ...
    from bot.handlers import menu_blogger
    await menu_blogger.stop(message, state)
